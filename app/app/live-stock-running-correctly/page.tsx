...
// ✅ NEW (for Planned & Finished logic)
...
export default function LiveStockPage() {
  ...
  const merged: (LiveStockRow & { finished_stock_qty?: number })[] = useMemo(() => {
    ...
    return base.map((m) => {
      ...
      const plannedRemaining = Math.max(0, plannedReq - finished); // ✅ minus FG from planned
      const unplannedAfter = receivedUnplanned - plannedRemaining; // ✅ minus planned from unplanned (can be negative)
    ...
    <div className="flex flex-col gap-3 p-4 md:p-5 text-[rgb(var(--text))]">
      ...
      >
        <div className="flex flex-col gap-1">
          <label className="text-[12px] font-semibold text-[rgb(var(--lynch))]">Plant</label>
          <select
            value={plantFilter}
            onChange={(e) => {
            ...
            className="h-9 min-w-[160px] rounded-xl border border-[rgb(var(--border))]
                       bg-[rgb(var(--card-bg))] px-2 text-[13px] outline-none
                       focus:ring-2 focus:ring-[rgb(var(--ring))]"
          >
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[12px] font-semibold text-[rgb(var(--lynch))]">Pack Type</label>
          <select
            value={packTypeFilter}
            onChange={(e) => {
              setPackTypeFilter(e.target.value);
              setCurrentPage(1);
            }}
            className="h-9 min-w-[160px] rounded-xl border border-[rgb(var(--border))]
                       bg-[rgb(var(--card-bg))] px-2 text-[13px] outline-none
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[12px] font-semibold text-[rgb(var(--lynch))]">Pack Size</label>
          <select
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[12px] font-semibold text-[rgb(var(--lynch))]">Stock Level %</label>
          <select
            value={colorRange}
            onChange={(e) => {
              setColorRange(e.target.value as any);
              setCurrentPage(1);
            }}
            className="h-9 min-w-[160px] rounded-xl border border-[rgb(var(--border))]
                       bg-[rgb(var(--card-bg))] px-2 text-[13px] outline-none
          ...
          >
            ...
            <option value="66_100">66% - 100%</option>
            <option value="33_66">33% - 66%</option>
            <option value="below_33">Below 33%</option>
          </select>
        </div>

        <div className="flex flex-col gap-1">
          <label className="text-[12px] font-semibold text-[rgb(var(--lynch))]">Rows</label>
          <select
            value={rowsPerPage}
            onChange={(e) => {
            ...
            className="h-9 min-w-[96px] rounded-xl border border-[rgb(var(--border))]
                       bg-[rgb(var(--card-bg))] px-2 text-[13px] outline-none
                       focus:ring-2 focus:ring-[rgb(var(--ring))]"
          >
                  ...
                  currentItems.map((item: any, idx) => {
                    ...
                    return (
                      <tr
                        ...
                        onDoubleClick={() => {
                          setActiveItemCode(String(item.item_code ?? "").trim());
                          setModalOpen(true);
                        }}
