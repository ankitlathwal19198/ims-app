...
export default function IMSMasterAddPage() {
  ...
  /** Smart Suggest (searchable dropdown) */
  const getFiltered = (opts: string[], q: string) => {
    const qq = norm(q).toLowerCase();
    const base = opts.filter(Boolean);
    if (!qq) return base.slice(0, 10);
    return base.filter((x) => x.toLowerCase().includes(qq)).slice(0, 10);
  };

  /** Controlled width wrapper (inputs not too wide) */
  const FieldWrap = ({
    children,
    k,
  }: {
  ...
  const SuggestInput = ({
    ...
    const filtered = useMemo(() => getFiltered(options, form[k]), [options, form, k]);
  ...
  const Section = ({
    ...
    >
      ...
      {/* controlled width + centered layout */}
